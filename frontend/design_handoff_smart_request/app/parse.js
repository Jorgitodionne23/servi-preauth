/* ─────────────────────────────────────────────────────────────────────────
   ...
   • engine 'ai'        → uses window.claude.complete, grounded by SERVI_CATALOG.
...
(function () {
  ...
  // ── AI engine (grounded by the catalog) ──────────────────────────────────
  ...
  async function ai(text) {
    if (!(window.claude && typeof window.claude.complete === 'function')) throw new Error('no-llm');
    const today = new Date();
    const todayStr = today.toISOString().slice(0, 10);
    const prompt = `You are SERVI's request-understanding engine. SERVI is an on-demand home-services platform in Mexico City. A user typed a service request in plain language. Map it to the closest service in SERVI's catalog and decide what brief follow-up details would help a specialist arrive prepared.
...
Respond with ONLY a JSON object (no prose, no markdown) of this exact shape:
{
  ...
  "summary": "<one short sentence restating the need in plain English, max 12 words>",
...
Rules: 1-3 followups, only ones that genuinely help; prefer questions with quick chip options; never ask for name, phone, address, date, or price (handled elsewhere). Keep questions under 8 words.`;
    ...
    const jsonStr = String(raw).slice(String(raw).indexOf('{'), String(raw).lastIndexOf('}') + 1);
