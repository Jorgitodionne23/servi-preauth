...
(function () {
  ...
  function composeHTML() {
    ...
    if (S.mode === 'text') {
      boxInner =
        '<textarea class="sr-ta" id="sr-ta" rows="3" spellcheck="false" placeholder="Describe what you need…  e.g. “' + esc(EXAMPLES[0]) + '”">' + esc(S.text) + '</textarea>' +
        (S.atts.length ? '<div class="sr-att-row">' + S.atts.map(function (a, i) {
          return '<div class="sr-att">' + (a.sample ? '<div class="sr-att__ph">' + I.camera(16) + '</div>' : '<img src="' + a.url + '" alt="">') +
      ...
      boxInner = '<button type="button" class="sr-box__back" data-action="mode:text">' + I.back(16) + 'Back to typing</button>' +
        (S.mode === 'voice' ? voicePanelHTML() : mediaPanelHTML());
    }

    return '<div class="sr-stage sr-stage--narrow sr-fade-in"><div class="sr-compose">' +
      '<div class="sr-eyebrow"><span class="sr-eyebrow__spark">' + I.spark(15) + '</span>SERVI Intelligence</div>' +
      '<h1 class="sr-title">What do you need done?</h1>' +
      '<p class="sr-sub">Describe it, show it, or say it — SERVI figures out the rest and finds your specialist.</p>' +
      '<div class="sr-box' + (S.mode !== 'text' ? ' sr-box--media' : '') + '" id="sr-box">' + boxInner + '</div>' +
      '<div class="sr-modes-head">Or request another way</div>' +
      '<div class="sr-modes">' + MODE_TILES.map(function (m) {
        return '<button type="button" class="sr-mode' + (S.mode === m.key ? ' on' : '') + '" data-action="mode:' + m.key + '">' +
          ...
          '<span class="sr-mode__label">' + m.label + '</span><span class="sr-mode__sub">' + m.sub + '</span></span></button>';
      }).join('') + '</div>' +
      '<div class="sr-browse"><span class="sr-browse__q">Not sure what to ask for?</span>' +
        '<button type="button" class="sr-browse__btn" data-action="browse-open">' + I.grid(16) + 'Browse all services</button></div>' +
      '</div></div>';
  }
  ...
  function voicePanelHTML() {
    ...
    if (r.phase === 'done') {
      return '<div class="sr-capture sr-capture--voice">' +
        ...
        '<div class="sr-capture__actions">' +
          btn('ghost', 'sm', 'Re-record', { action: 'voice-reset', iconLeft: I.mic(16) }) +
          btn('accent', 'sm', 'Use this recording', { action: 'voice-use', iconRight: I.arrow(16) }) + '</div>' +
        '<p class="sr-capture__note">SERVI will transcribe and understand your note — we’ll only ping you on WhatsApp if something’s unclear.</p>' +
        '</div>';
    }
    ...
    return '<div class="sr-capture sr-capture--voice">' +
      ...
      '<div class="sr-voice-meta">' + (rec
        ...
        : '<span class="sr-capture__hint">Tap to record · up to ' + fmtTime(SETTINGS.voiceLimit) + '</span>') + '</div>' +
      '</div>';
  }
  ...
  function mediaPanelHTML() {
    ...
    if (!isPhotos && r.phase === 'vidrec') {
      return '<div class="sr-capture sr-capture--video"><div class="sr-vidrec">' +
        ...
        '<p class="sr-capture__hint">Describe what you need while you film it.</p></div>' +
        '<div class="sr-capture__actions">' + btn('accent', 'sm', 'Stop recording', { action: 'vid-stop', iconLeft: I.stop(16) }) + '</div></div>';
    }
    ...
    if (has) {
      body = '<div class="sr-thumbs">' + S.media.map(function (it, i) {
        ...
        return '<div class="sr-thumb">' + inner + '<button type="button" class="sr-thumb__x" data-action="media-remove:' + i + '">' + I.x(13) + '</button></div>';
      ...
      body = '<div class="sr-drop"><div class="sr-drop__icon">' + (isPhotos ? I.camera(26) : I.video(26)) + '</div>' +
        '<p class="sr-drop__title">' + (isPhotos ? 'Take or upload photos of the problem' : 'Record or upload a short video') + '</p>' +
        '<div class="sr-drop__btns">' + btn('secondary', 'sm', isPhotos ? 'Choose photos' : 'Upload video', { action: 'media-upload', iconLeft: I.upload(16) }) +
          (!isPhotos ? btn('secondary', 'sm', 'Record now', { action: 'media-record', iconLeft: I.video(16) }) : '') + '</div>' +
        '<button type="button" class="sr-sample" data-action="media-sample">Try a sample</button></div>';
    }
    var note = isPhotos
      ? 'SERVI reads your photos to understand the job — we’ll confirm any details on WhatsApp.'
      : 'Our specialists review your clip in detail; we’ll reach out on WhatsApp if anything’s unclear.';
    return '<div class="sr-capture">' + body +
      '<div class="sr-capture__foot"><p class="sr-capture__note">' + note + '</p>' +
        (has ? btn('accent', 'sm', isPhotos ? ('Continue with ' + S.media.length + ' photo' + (S.media.length > 1 ? 's' : '')) : 'Continue with video', { action: 'media-use', iconRight: I.arrow(16) }) : '') +
      '</div></div>';
  }
  ...
  function understandingHTML() {
    ...
    return '<div class="sr-understand sr-fade-in"><div class="sr-understand__top">' +
      ...
      '<button type="button" class="sr-link" data-action="open-picker">' + I.edit(14) + 'Not quite? Change service</button></div>';
  }

  function mediaReceivedHTML() {
    var n = S.media.length;
    var map = {
      photos: { ic: I.camera(22), label: n + ' photo' + (n > 1 ? 's' : '') + ' received', desc: 'Your photos tell us what we need. We’ll review them and confirm any details on WhatsApp.' },
      video: { ic: I.video(22), label: 'Video received', desc: 'Our specialists will review your clip in detail and reach out on WhatsApp if anything’s unclear.' },
    };
    ...
    return '<div class="sr-understand sr-understand--media sr-fade-in"><div class="sr-understand__top">' +
      ...
      '<div class="sr-understand__eyebrow">' + I.check(13) + 'Request captured</div>' +
  ...
  function followupsHTML() {
    ...
    return '<div class="sr-card sr-fade-in"><div class="sr-card__head"><h3 class="sr-card__title">A few quick details</h3>' +
      '<span class="sr-card__opt">Optional — helps your specialist arrive ready</span></div><div class="sr-fups">' +
      f.map(function (q, i) {
        var key = q.key || ('q' + i);
        var control = (q.chips && q.chips.length)
          ...
          : '<input class="sr-fup__input" data-fup="' + key + '" placeholder="Type your answer…" value="' + esc(S.answers[key] || '') + '">';
  ...
  function whenWhereHTML() {
    ...
    return '<div class="sr-card sr-fade-in"><div class="sr-card__head"><h3 class="sr-card__title">When &amp; where</h3></div>' +
      '<div class="sr-when">' +
        '<button type="button" class="sr-radio' + (S.when === 'asap' ? ' sr-radio--on' : '') + '" data-action="when:asap">' +
          '<span class="sr-radio__ic">' + I.bolt(18) + '</span><span class="sr-radio__txt"><span class="sr-radio__label">As soon as possible</span>' +
          '<span class="sr-radio__desc">We’ll dispatch the next available specialist</span></span></button>' +
        '<button type="button" class="sr-radio' + (S.when === 'schedule' ? ' sr-radio--on' : '') + '" data-action="when:schedule">' +
          '<span class="sr-radio__ic">' + I.calendar(18) + '</span><span class="sr-radio__txt"><span class="sr-radio__label">Schedule for later</span>' +
          '<span class="sr-radio__desc">' + (S.dateLabel && S.when === 'schedule' ? 'Detected: ' + esc(S.dateLabel) : 'Pick a day and time') + '</span></span></button>' +
      '</div>' +
      ...
      '<div class="sr-where"><label class="sr-where__label">' + I.pin(15) + 'Service address</label>' +
        '<div class="sr-where__row"><input class="sr-input" id="sr-addr" placeholder="Street, number, neighborhood" value="' + esc(S.address) + '">' +
  ...
  function summaryHTML() {
    ...
    var whenStr = S.when === 'asap' ? 'As soon as possible' : (S.date ? fmtDate(S.date) + (S.time ? ' · ' + S.time : '') : 'Scheduled');
    var mediaLabel = S.mode === 'voice' ? 'Voice note' : S.mode === 'video' ? 'Video clip' : S.mode === 'photos' ? (S.media.length + ' photo' + (S.media.length > 1 ? 's' : '')) : null;
    ...
    return '<div class="sr-sum"><div class="sr-sum__head"><span class="sr-sum__emoji">' + (req.emoji || '✨') + '</span>Your request</div>' +
      '<div class="sr-sum__rows">' +
        row('Service', svcVal) +
        ((S.mode !== 'video' && req.subLabel) ? row('Category', req.categoryLabel) : '') +
        (details.length ? row('Details', details.join(' · ')) : '') +
        row('When', whenStr) +
        row('Where', S.address || '—') +
      '</div></div>';
  }
  ...
  function nextStepsHTML() {
    ...
    var steps = [
      { ic: I.users(17), t: 'We match your specialist', d: 'Our team finds the nearest verified pro for your exact request.' },
      { ic: I.tag(17), t: 'We confirm the price', d: 'You get a clear price for the job — no surprises, nothing charged yet.' },
      { ic: I.whatsapp(17), t: 'We reach out on WhatsApp', d: 'We message you to lock in the specialist and the time.' },
    ];
    return '<div class="sr-next"><div class="sr-next__head"><span class="sr-next__title">What happens next</span>' +
      badge('neutral', I.clock(12) + ' Usually within ~15 min', 'sr-next__eta') + '</div>' +
      ...
      '<div class="sr-next__trust">' + I.shield(14) + 'Verified specialists · Price confirmed before any charge · Secured with Stripe</div></div>';
  }

  function buildHTML() {
    var left = '<div class="sr-pane-left"><button type="button" class="sr-editlink" data-action="reset">' + I.back(15) + 'Edit request</button>' +
      (S.thinking
    ...
    var right = '<div class="sr-pane-right"><div class="sr-rail">' + summaryHTML() + nextStepsHTML() +
      btn('accent', 'lg', 'Send request', { action: 'submit', block: true, id: 'sr-submit', disabled: S.thinking || !canSend, iconRight: I.send(18) }) +
      '<p class="sr-rail__fine">You won’t be charged now. We confirm the price before anything happens.</p></div></div>';
  ...
  function successHTML() {
    var id = 'SV-' + Math.floor(100000 + Math.random() * 899999);
    var whenStr = S.when === 'asap' ? 'As soon as possible' : (S.date ? fmtDate(S.date) + (S.time ? ' · ' + S.time : '') : 'Scheduled');
    var what = (S.mode === 'text' || S.mode === 'voice' || S.mode === 'photos') && S.req.service ? S.req.service
      : (S.mode === 'voice' ? 'Voice note' : S.mode === 'video' ? 'Video request' : S.media.length + ' photos');
    return '<div class="sr-stage sr-stage--narrow sr-fade-in"><div class="sr-success">' +
      ...
      '<h2 class="sr-success__title">Request sent</h2>' +
      '<p class="sr-success__sub">We’ll text you on WhatsApp shortly to confirm your specialist and the price.</p>' +
      '<div class="sr-success__card"><div class="sr-success__id">Request <strong>#' + id + '</strong></div>' +
        '<div class="sr-success__rows"><div><span>Request</span>' + esc(what) + '</div>' +
        '<div><span>When</span>' + esc(whenStr) + '</div><div><span>Where</span>' + esc(S.address || '—') + '</div></div></div>' +
      '<div class="sr-success__actions">' + btn('accent', 'md', 'Open WhatsApp chat', { action: 'open-whatsapp', iconLeft: I.whatsapp(18) }) +
        btn('secondary', 'md', 'New request', { action: 'reset' }) + '</div></div></div>';
  }

  // ════════════════════════════ TOP BAR + MODALS ═══════════════════════════
  function topbarHTML() {
    var loc = S.address.length > 22 ? DEFAULT_ADDR : S.address;
    return '<header class="sr-top"><div class="sr-top__in">' +
      ...
      '<div class="sr-top__right"><span class="sr-top__loc">' + I.pin(14) + esc(loc) + '</span>' +
      '<span class="sr-lang"><button class="on">EN</button><button>ES</button></span>' +
      '<button type="button" class="sr-iconbtn sr-iconbtn--outline" data-action="open-tweaks" aria-label="Settings" style="width:38px;height:38px">' + I.sliders(17) + '</button>' +
      '</div></div></header>';
  }
  ...
  function renderPicker() {
    ...
    var html = '<div class="sr-modal__overlay" data-action="modal-close"><div class="sr-modal" data-stop>' +
      '<div class="sr-modal__head"><h3 class="sr-modal__title">Choose the service</h3>' +
  ...
  function openBrowse() {
    var cats = CAT();
    var html = '<div class="sr-modal__overlay" data-action="modal-close"><div class="sr-modal" data-stop>' +
      '<div class="sr-modal__head"><h3 class="sr-modal__title">Browse all services</h3>' +
      '<button type="button" class="sr-modal__close" data-action="modal-close">' + I.x(18) + '</button></div>' +
      '<div class="sr-modal__body"><p style="margin:0 0 16px;font-size:14px;color:var(--text-secondary)">Prefer to look around? Open the full catalog and pick a service yourself.</p>' +
  ...
  // ── PRODUCTION-SHAPED PAYLOAD (mirrors window.submitBooking) ──
  ...
  function submit() {
    // Auth gate hook (optional) — mirrors window.submitBooking behavior.
    if (typeof window.__user === 'object' && window.__user === null && typeof window.openAuthModal === 'function') { window.openAuthModal('login'); return; }
    var payload = buildPayload();
    var btnEl = document.getElementById('sr-submit'); if (btnEl) { btnEl.disabled = true; btnEl.querySelector('span').textContent = 'Sending…'; }
    ...
    if (API) {
      fetch(API + '/api/service-requests', { method: 'POST', headers: headers, body: JSON.stringify(payload) })
        ...
        .then(function () { S.phase = 'success'; render(); window.scrollTo({ top: 0 }); })
        .catch(function () { S.phase = 'success'; render(); window.scrollTo({ top: 0 }); }); // demo-friendly
    } else {
      // standalone design-review mode — log the production payload, show success
      try { console.log('[SERVI] service-request payload →', payload); } catch (e) {}
      S.phase = 'success'; render(); window.scrollTo({ top: 0 });
    }
  }

  function reset() {
    stopWave(); if (S.rec && S.rec.timer) clearInterval(S.rec.timer);
    S.phase = 'compose'; S.mode = 'text'; S.text = ''; S.atts = []; S.media = []; S.rec = null;
    S.thinking = false; S.req = { emoji: '✨', categoryLabel: 'Custom request' }; S.answers = {};
    S.when = 'asap'; S.date = ''; S.time = ''; S.dateLabel = ''; S.address = DEFAULT_ADDR;
    render();
  }
  ...
  document.addEventListener('click', function (e) {
    ...
    switch (cmd) {
      ...
      case 'attach-photos': pickFiles('image/*', true, 'environment', function (files) { S.atts = S.atts.concat(files.slice(0, 4).map(function (f) { return { url: URL.createObjectURL(f) }; })).slice(0, 4); render(); }); break;
      ...
      case 'media-upload': pickFiles(S.mode === 'photos' ? 'image/*' : 'video/*', S.mode === 'photos', S.mode === 'photos' ? 'environment' : null, function (files) {
          if (S.mode === 'photos') S.media = S.media.concat(files.slice(0, 5).map(function (f) { return { kind: 'photo', url: URL.createObjectURL(f) }; })).slice(0, 5);
      ...
      case 'use-loc': var loc = document.querySelector('.sr-loc'); if (loc) loc.classList.add('busy'); setTimeout(function () { S.address = 'Av. Santa Fe 482, Santa Fe, CDMX'; render(); }, 900); break;
      case 'chip': var key = arg, val = t.getAttribute('data-val'); S.answers[key] = (S.answers[key] === val ? '' : val); render(); break;
      case 'submit': submit(); break;
      case 'reset': reset(); break;
      case 'browse-open': openBrowse(); break;
      ...
      case 'open-whatsapp': window.open('https://wa.me/', '_blank'); break;
  ...
  document.addEventListener('keydown', function (e) { if (e.target.id === 'sr-ta' && e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); submitText(); } if (e.key === 'Escape') closeModal(); });
