...
export async function initDb() {
  await pool.query(`
    ...
    -- Service Calendar (recurring services): when a request is auto-generated by the recurring
    -- cron, this links it back to its plan. recurring_occurrence_date is the calendar date the
    -- occurrence represents — together they give the idempotency key the cron dedupes on.
    ALTER TABLE service_requests ADD COLUMN IF NOT EXISTS recurring_service_id TEXT;
    ALTER TABLE service_requests ADD COLUMN IF NOT EXISTS recurring_occurrence_date DATE;
    CREATE UNIQUE INDEX IF NOT EXISTS idx_service_requests_client_request_id
      ON service_requests(client_request_id)
      WHERE client_request_id IS NOT NULL;
    -- One materialized request per (plan, occurrence date) — the cron's safety net against
    -- double-generation on overlapping runs.
    CREATE UNIQUE INDEX IF NOT EXISTS idx_service_requests_recurring_occurrence
      ON service_requests(recurring_service_id, recurring_occurrence_date)
    ...
    -- Service Calendar — recurring service plans. A client sets up a repeating service (e.g. a
    -- weekly cleaning with their trusted specialist); the recurring cron materializes one
    -- service_request per due occurrence (lead_time_days ahead) into the normal admin pipeline.
    -- This table holds only the *intent/schedule* — no money moves here. Each generated request
    -- becomes a normal order through the existing flow. No hard FK to providers so a removed
    -- specialist never deletes the plan; the preference is re-validated at materialization time.
    CREATE TABLE IF NOT EXISTS recurring_services (
      id                      TEXT PRIMARY KEY,
      user_id                 TEXT NOT NULL REFERENCES auth_users(id) ON DELETE CASCADE,
      category                TEXT NOT NULL,
      description             TEXT,
      provider_id             TEXT,                 -- preferred specialist (optional)
      provider_name           TEXT,
      frequency               TEXT NOT NULL,        -- daily | weekly | biweekly | monthly | custom
      interval_count          INTEGER DEFAULT 1,    -- custom = every N days
      weekdays                TEXT,                 -- CSV of 0..6 (0=Sun) for weekly/biweekly
      day_of_month            INTEGER,              -- 1..31 for monthly
      preferred_time          TEXT,                 -- 'HH:MM' local (America/Mexico_City)
      service_address         TEXT,
      service_address_details TEXT,                 -- JSON (structured CDMX address)
      start_date              DATE NOT NULL,
      end_date                DATE,                 -- NULL = open-ended
      lead_time_days          INTEGER DEFAULT 3,    -- days before an occurrence to create its request
      status                  TEXT NOT NULL DEFAULT 'active',  -- active | paused | canceled
      last_run_at             TIMESTAMPTZ,
      last_occurrence_date    DATE,                 -- most recent occurrence already materialized
      next_occurrence_date    DATE,                 -- cached next upcoming date (for quick listing)
    ...
    CREATE INDEX IF NOT EXISTS idx_recurring_services_user
      ON recurring_services (user_id, status, created_at DESC);
    CREATE INDEX IF NOT EXISTS idx_recurring_services_active
      ON recurring_services (status, next_occurrence_date)
      WHERE status = 'active';
