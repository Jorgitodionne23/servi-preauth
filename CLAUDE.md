...
- Session refresh + revocation: `POST /api/auth/refresh` rotates a near-expired JWT (new `jti`; the frontend refreshes proactively when <6h remain). Logout, account deletion, and refresh rotation write jti tombstones to `revoked_sessions`; phone/email changes set `auth_users.sessions_invalidated_before`, killing every session except the one that made the change (exempt jti). Account deletion requires fresh re-auth (`X-Firebase-Reauth-Token`, 60s window) and deleted users' tokens fail immediately (token verification checks user-row existence).
