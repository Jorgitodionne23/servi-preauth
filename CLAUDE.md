...
- `native-app-reference/` — High-fidelity Expo + React Native + TypeScript prototype of the **customer** app. Mocked data only; isolated from the web app. Designed as the mirror of `partner-app-reference/`: they share three order IDs (`SV-204815`, `SV-204766`, `SV-204701`), the real pricing engine, and a frozen CDMX-fixed demo clock, so the same order can be reviewed from both sides. Shows masked specialist names, a live on-site check-in timeline, an aggregate satisfaction score, and an optional tip. See its own `README.md`.
- `partner-app-reference/` — High-fidelity Expo + RN + TS prototype of the **specialist (SERVI Partner)** app: onboarding + verification, job offers with accept/decline, on-site check-in milestones, mid-job price adjustments, and self-serve **earnings + payouts**. Mocked data only, no Stripe/Firebase. See its `README.md` (incl. how to test without an Apple Developer account).
- **`INTEROP.md`** (repo root) — the production spec for the two-app system: type↔table mapping, which backend routes exist vs need building, schema additions, and the Stripe Connect recommendation.
- **Three-way pricing sync:** `backend/pricing.mjs`, `native-app-reference/src/data/pricing.ts`, and `partner-app-reference/src/data/pricing.ts` must stay numerically identical. The two `pricing.ts` files are byte-identical, along with the shared theme/UI/i18n/clock files — `scripts/check-app-sync.mjs` (`npm run check:sync`) enforces this.
